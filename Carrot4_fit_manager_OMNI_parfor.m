close all;
clear all;
load Carrot4_data_piped(4th1cut).mat;
format shortg;
startTime = tic;
nSubject = 12;
...
for i = 1:length(ExperimentalTrials)
    if ExperimentalTrials(i).dump == 0 && ExperimentalTrials(i).dv ~= 0 &&...
            ExperimentalTrials(i).t_total-ExperimentalTrials(i).manipOnset >= simLength && i ~= 1059
...
if strcmp(method,'LOO')
    ...
    for subject = 1:13
        for d0 = [1 3 6]
            for dv = [-0.3 0.3]
                PoolList(end+1).subjCon = ['subj' num2str(subject) '_' num2str(d0) num2str(dv)];
                for i = 1:length(DATA_SET)
                    if DATA_SET(i).subject == subject && DATA_SET(i).d0 == d0 &&...
                            DATA_SET(i).dv == dv
...
parfor ii = 1:size(indexList,2)
    ...
    if ~strcmp(model,'nullModel')
        zFunction = @(p) train(model, p, w, h, delay, TRAIN_SET, criterion, variable, simLength, Hz);
    ...
    iResults = test(TEST_SET, model, iIter, pIter, w, h, delay, simLength, Hz);
...
save([experiment,'_fit_pertTrials_',method,num2str(nIter),'_',criterion,'_',variable,'_',char(join(time(1:5),'-')),'.mat'],...
     ...
     'simLength', 'Results', 'ResultsSummary', 'totalTime');
% pertTrials means perturbation trials dv = -0.3 or 0.3

%% Functions

function index = train(model, p, w, h, delay, TRAIN_SET, criterion, variable, simLength, Hz)
...
function iResults = test(TEST_SET, model, iIter, p, w, h, delay, simLength, Hz)
iResults = TEST_SET;
