...
%% loading
subFolder = 'Carrot4_rawData\';

% load test trials
loading1 = dir(strcat(subFolder,'*position_subj*')); % load all txt files in directory of code
...
for iTrial = 1:length(loading1) % iterate through all trials
    ...
    % read condition
    ExperimentalTrials(iTrial).d0 = str2double(loading1(iTrial).name(34));
    ExperimentalTrials(iTrial).dv = str2double(loading1(iTrial).name(37:40));

    fileName_ = strcat(subFolder, 'Carrot4_input\Carrot4_subject', num2str(ExperimentalTrials(iTrial).subject,'%02d'),...
        '_conditions.csv');
    ...
    % comparing the d0 and dv of that trial
    if ExperimentalTrials(iTrial).d0 == conditionFile(ExperimentalTrials(iTrial).trial,2) &&...
    ...
    ExperimentalTrials(iTrial).t_total = raw_traj(end,5);
    ExperimentalTrials(iTrial).raw_traj = raw_traj;
...
% load error notes
loading3 = dir(strcat(subFolder,'*position*DUMP*')); % load all txt files in directory of code
Dump = struct;
...
for iTrial = 1:length(ExperimentalTrials)
    ...
    d0 = ExperimentalTrials(iTrial).d0;
    ...
    % Butterworth filter
    [vOutput, outFiltered, outExtended] = Carrot4_filter_butter (Hz,ExperimentalTrials(iTrial).inter_traj(:, 4),cutoff,order,pad);
    ...
    % mark dump trials by suspicious speed
...
%% save
save(['Carrot4_data_piped(', num2str(order), 'th', num2str(cutoff), 'cut).mat'], 'ExperimentalTrials', 'Dump');
