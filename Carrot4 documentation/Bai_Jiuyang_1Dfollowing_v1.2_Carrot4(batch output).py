...
EXPERIMENT = '1Dfollowing_v1.2_Carrot4'
NICKNAME = 'Carrot4'
...
# Use Oculus Rift
	...
	link.preTrans([0, -0.055, -0.08]) # [right, up, front]
	link.preEuler([0, 1, 0]) # [yaw, pitch(down), row]
...
def masterLoop(num):
	...
	if is_freewalk == True:
		...
		if DATA_COLLECT and data_collect:
			...
			data_batch_pos = data_batch_pos + strData
			# Orientation: Yaw, Pitch, Row, time stamp
			data = [cur_rot[0], cur_rot[1], cur_rot[2]]
			strData = [str(round(t,4)) for t in data+[time]]
			strData = ','.join(strData)+'\n'
			data_batch_ori = data_batch_ori + strData
...
# +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
		...
		# 00 05 End Freewalk Trial: Close out the trial and reset values for next practice trial or start Experiment
			...
			fileName = OUTPUT_DIR + NICKNAME + '_position' + '_freewalk' + \
			'_subj' + subject + '_s' + str(freewalk_session) + '_trial' + str(trial_num).zfill(3) + '.csv'
			file = open(F, 'a')
			file.write(data_batch_pos)
			file.close()
			fileName = OUTPUT_DIR + NICKNAME + '_orientation' + '_freewalk' + \
			'_subj' + subject + '_s' + str(freewalk_session) + '_trial' + str(trial_num).zfill(3) + '.csv'
			file = open(fileName, 'a')
			file.write(data_batch_ori)
			file.close()
...
# +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
	...
	####################
		...
		if trial_stage == 'pretrial_02_01':
			...
			condition = ', '.join([conditions[trial_num][1], conditions[trial_num][3].zfill(4)])
...
# ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
		...
		# 02 05 End Trial: Close out the trial and reset values for next trial
			...
			fileName = OUTPUT_DIR + NICKNAME + '_position_' + 'subj' + subject + '_trial' + str(trial_num).zfill(3) + '_' + condition + '.csv'
			file = open(fileName, 'a')
			file.write(data_batch_pos)
			file.close()		
			fileName = OUTPUT_DIR + NICKNAME + '_orientation_' + 'subj' + subject + '_trial' + str(trial_num).zfill(3) + '_' + condition + '.csv'
			file = open(fileName, 'a')
			file.write(data_batch_ori)
			file.close()
