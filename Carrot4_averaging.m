...
% compute confidence interval and divid by n to get the average
...
iModel = 3; % indicate the model you want to plot by its index in modelList
iSubject = 0; % 0 is the average of all subjects

for i = 1:length(ResultsAveraged)
    model = ResultsAveraged(i).model;
    subject = ResultsAveraged(i).subject;
    d0 = ResultsAveraged(i).d0;
    ...
    if strcmp(model, modelList(iModel)) && subject == iSubject
        ...
        title(['d0=', num2str(d0), '  dv=', num2str(dv), ' ', model{1}]);
        axis([-0.5 6 0 1.6]);
        xlabel('Time');
        ylabel('Speed');
        saveas(gcf,['d0=', num2str(d0), '  dv=', num2str(dv), ' ', model{1}, '.png']);
    end
end
